...
class EventViewer {
    ...
    async getData() {
        ...
        const eventRef = ref(database, 'events/' + this.eventId);
        const snapshot = await get(eventRef);
        if (snapshot) {
            const eventData = snapshot.val();
            console.log("eventData:", eventData);
            this.displayEventData(eventData);
        }
    }

    private displayEventData(eventData: any) {
        /*About*/
        this.setElementTextContent("about", eventData?.about, "not set");
        /*Title*/
        this.setElementTextContent("title", eventData?.title, "not set");

        //#region Colleges and Majors
        let edColleges = eventData?.college ?? "Not set";
        let edMajors = eventData?.majors ?? "Not set";

        if (edMajors !== "Not set") {
            edMajors = this.mapMajors(edMajors);
            this.setElementTextContent("majors", edMajors.join(', '), "Not set");
        }
        else{
            this.setElementTextContent("majors", edMajors, "Not set"); 
        }
        if (edColleges !== "Not set") {
            edColleges = this.mapColleges(edColleges);
            this.setElementTextContent("college", edColleges.join(', '), "Not set");
        }
        else{
            this.setElementTextContent("college", edColleges.join(', '), "Not set");
        }
       ...
       // formColleges!.textContent = jdColleges.join(', ');
        ...
        //Type
        this.capitalizeAndSetContent("type", eventData?.type, "Not Set");
        //Time
        this.capitalizeAndSetContent("time", eventData?.time, "Not Set");
        //Date
        this.capitalizeAndSetContent("date", eventData?.date, "Not Set");
        //Year
        let jdGLs = eventData?.gradeLevels ?? "Not set";
        if (jdGLs !== "Not set") {
